...
本分支提供 AIOS 一小时连续场景验收基线：时间从 2027-04-25 13:00 正常推进，系统运行时主动投放短信、来电、提醒和通知，Agent 将事件归入多个任务并驱动手机端任务界面。LLM 深度编排改造不包含在本分支，后续会从该验收基线另起分支推进。

### AIOS 多任务界面
- 首屏进入任务体验界面，13:05 前会话区保持空白，只显示正常推进的时间和底部交互区。
- 左上角侧边栏承载任务卡片，默认收起；卡片按最后活跃时间排序，点击任务只切换详情，不改变排序；长按可用钉子图标置顶。
- 任务详情保留会话区、候选动作、单行进程区、底部交互区和可展开蓝图区。
- 蓝图区只显示当前任务的运行日志，日志严格按出现顺序追加，不排序、不去重；新增日志会自动展开并滚到最新。
- 时间区点击用于测试快进：从正常走时切到“快进到下一事件”，触发下一条系统运行时事件后自动恢复正常走时。
- AI 工作动态图标位于进程区后段，点击展开蓝图区；时间区不再负责展开蓝图区。
- 系统提醒以应用内浮层出现，20 秒无人处理会自动关闭。
- 来电接通后会用通话层覆盖会话区和蓝图区，保留一段可见时间后再回到任务流。

### 一小时场景流
- 全局事件脚本位于 `core/data/src/main/assets/skills/md/one-hour-aio/SYSTEM_RUNTIME.json`。
- 场景路由位于 `:scenarios:one-hour-flow`，只负责把系统运行时事件分发给对应任务面。
- 已覆盖任务线包括：麒麟洗护、家庭采购、健康补给、冷链收货。
- 13:05 宠物店来信触发洗护任务，用户选择 `可以` 后才会继续司机协调、送下楼提醒、到店和服务进度；选择 `不改了` 后宠物后续事件被抑制。
- 13:09 Ella 来电触发通话层，13:11 通话结束后创建家庭采购任务。
- 13:20 以后继续投放物业提醒、宠物店进度、Ella 采购补充、冷链保管、健康补给候选和超市订单锁定等事件。

### 场景与核心边界
- `SystemRuntime` 只扮演手机系统与外部世界，负责收到短信、来电、提醒触发、通知、联系人、当前定位和设备状态。
- Agent 侧负责事件理解、任务创建、任务编排、地点理解、服务查询、支付/记账和用户决策点管理。
- 场景内容只落在 `scenarios/*` 和 `core/data/src/main/assets/skills/md/*`；`feature:chat` 只保留通用 UI 与多 session 承载；`core:domain` 不承载具体场景文案。
- `pet-grooming`、`family-shopping`、`coldchain-delivery`、`health-supply` 输出统一的 task surface，由 `one-hour-flow` 组合成一小时事件流。

### 验证方式
- 单元测试覆盖一小时脚本事件白名单、`可以 / 不改了` 两条宠物分支、来电层和 13:20 后独立任务路由。
- 静态边界扫描需要确认 `feature/chat/src/main/java` 与 `core/domain/src/main/java` 不出现具体场景实体名。
- 构建验收使用 `./gradlew assembleDebug`。
- 真机快进验收覆盖 13:05、13:08、13:09、13:11、13:20、13:22、13:31、13:36、13:45、14:00。
