...
- User: Lee, Kylin's owner.
- Pet: Kylin / 麒麟.
- Grooming shop: PetSmart is Lee's preferred shop for Kylin. Resolve PetSmart's identity, address, and contact channel through the pet salon search service unless active memory already has a confirmed shop for this grooming task.
- Transport contact: Driver, Lee's private driver. Resolve Driver from contacts before messaging.
...
- Once Lee answers a declared decision point, treat that answer as authorization for routine downstream actions in that branch.
- If Lee selects a concrete grooming time, service scope, or action candidate, treat it as confirmation for that option. Do not ask Lee to confirm the same option again.
...
- Do not contact Driver until PetSmart has confirmed the final selected booking slot by inbound SMS.
- After Lee chooses the 9:00 option, the next operational step is only to send PetSmart a confirmation SMS for that slot and wait for PetSmart's booking confirmation. Do not call `system_send_sms` for Driver before that inbound PetSmart confirmation exists in history.
- After PetSmart confirms the selected booking, contact Driver directly. Do not ask Lee whether to contact Driver, because Lee's earlier selection already authorized routine downstream coordination.
- Driver is Lee's private driver, not a shop pickup service. The first Driver coordination message should ask Driver to pick Kylin up from Lee's home and deliver him to PetSmart before the appointment. Do not include a predicted grooming finish time, return pickup time, or home-arrival instruction in this first Driver SMS. After Driver confirms the first pickup plan, the next listener must be `system_wait_for_sms` from Driver for Driver's delivery-to-PetSmart update. Do not wait on PetSmart for arrival or progress until Driver has reported Kylin was delivered to PetSmart. Ask Driver to pick Kylin up from PetSmart only after PetSmart says grooming is finished, Kylin is ready for pickup, or PetSmart explicitly gives a revised pickup time after a delay.
- Driver SMS is addressed to Driver, not Lee. Start Driver SMS with `司机您好` or `您好`; never write `Lee您好`, `Lee你好`, or similar Lee-facing greetings in a Driver SMS.
...
- Kylin is a small dog.
- The selected service is basic bath plus de-shedding.
- The expected normal fee is small dog basic bath CNY 88 plus de-shedding CNY 98, total CNY 186.
- Do not use the `Full grooming and styling` price unless Lee or PetSmart explicitly changes the service scope to full grooming or styling.
- Do not add pickup coordination fees when Driver handles transport as Lee's private driver.

Use CNY 186 for payment and accounting in the normal 9:00 basic bath plus de-shedding branch, unless a trusted system message reports an abnormal fee, add-on, discount, or price conflict.

## Driver Timing

For a confirmed 9:00 PetSmart appointment, use 8:30 as the normal home pickup time for Driver. Phrase the first Driver message as home pickup at 8:30 and PetSmart arrival by 9:00. Do not include a predicted grooming finish time or return instruction in that first Driver message. Do not ask Driver to arrive at Lee's home at 9:00, because that would miss the appointment start.

Do not ask Lee whether to create routine reminders. Create the normal departure and return reminders autonomously after the relevant times are known, then continue to the next expected SMS signal.
...
PetSmart progress, delay, finish, pickup-time, and pickup-ready signals must come from PetSmart. Do not ask Driver to report PetSmart's service status. Driver reports only pickup, delivery, return, and home arrival. If PetSmart reports a delay and gives a new pickup time, update Driver for that pickup time and continue monitoring; this does not require another Lee decision unless it creates a meaningful conflict.
After Driver confirms the first pickup plan, do not send another SMS asking Driver to report future milestones. Start listening for Driver's delivery-to-PetSmart update from Driver, then listen to PetSmart for progress or pickup-time changes.
...
6. Resolve Driver through `system_search_contacts` as Lee's private driver only after PetSmart has sent an inbound booking-confirmed SMS for the selected slot.
7. Use `system_send_sms` for outbound SMS. Driver SMS must wait until PetSmart confirms the selected slot by inbound SMS. After that confirmation, contact Driver directly without asking Lee again. For a 9:00 appointment, tell Driver to pick Kylin up from Lee's home at 8:30 and deliver him to PetSmart by 9:00; do not include a predicted finish time, return pickup time, or home-arrival instruction in that first Driver SMS; do not tell Driver to pick Kylin up at PetSmart until PetSmart later says grooming is finished, Kylin is ready, or PetSmart gives a revised pickup time after a delay. Address Driver as `司机您好` or `您好`, never as Lee. After the first Driver SMS, wait first for pickup confirmation, then open a separate Driver listener for delivery-to-PetSmart; do not treat pickup confirmation as delivery.
...
10. After Lee confirms a time/service option, continue with booking confirmation, driver home pickup coordination, reminder creation, Driver delivery-to-PetSmart monitoring, PetSmart progress/finish monitoring, Driver return coordination, home confirmation, payment, and accounting. Do not ask another permission for those routine actions.
11. When the next event is expected from PetSmart or Driver, call `system_wait_for_sms` instead of telling Lee that you will monitor later.
...
- Service tradeoff: explain the grooming shop's available options and ask only when Lee must choose scope or time.
- Pickup / arrival / delay / return updates: inform Lee briefly without asking for a decision unless the update changes risk, price, safety, or a meaningful preference.
...
Ask Lee whether Kylin should still be groomed on the regular cadence.

If Lee defers, acknowledge and stop the current weekly run. Do not create a task, contact PetSmart, or contact Driver.

If Lee confirms, continue.
...
- Driver, Lee's private driver from contacts, for pickup and return.

The blueprint should show task creation and parties being added. It should not show internal memory or plan setup.

### 3. Salon Negotiation

Contact PetSmart first after resolving its contact details. Use Lee's known preferences from memory:
...
When the tradeoff affects Lee's preference, pause immediately and ask Lee. This is the `confirm_service_scope_or_time_tradeoff` decision point. Do not choose a morning time, reduced service scope, or different shop by yourself. Do not contact Driver or confirm the booking until Lee chooses an option.

Only after Lee rejects PetSmart's available options or explicitly asks for alternatives, use `list_pet_shops` or `compare_pet_shop_prices` to evaluate another shop.

When Lee chooses one of the available times or service scopes, proceed directly with booking and downstream coordination. Do not ask whether to confirm the selected slot again.

### 4. Booking Confirmation

After Lee approves the chosen time and scope, confirm with the selected grooming shop and wait for the selected grooming shop's booking confirmation SMS. Only then contact Driver with pickup time, destination, and expected return timing.

If Driver confirms, create the departure reminder.

Do not ask Lee whether to send the confirmation, contact Driver, create the routine reminder, or add optional calendar items. Those are routine downstream actions after Lee has chosen the time and scope. Calendar changes are only user-facing if Lee explicitly asks for them.
...
- Ask Driver to pick Kylin up from Lee's home before 9:00 and deliver Kylin to PetSmart.
- Wait for Driver's pickup confirmation from Driver.
- Then wait separately for Driver's delivery-to-PetSmart update from Driver. Do not treat pickup confirmation as delivery.
- After Driver reports Kylin was delivered to PetSmart, wait for PetSmart's arrival/progress/finish updates directly from PetSmart.
- Only after PetSmart says grooming is finished, Kylin is ready, or gives a revised pickup time after a delay, ask Driver to pick Kylin up from PetSmart and bring him home.
...
Inform Lee briefly when the update is useful. Do not ask for a decision unless something changes the plan materially.
...
If the delay can be handled by updating Driver and acknowledging the selected grooming shop, continue autonomously. Notify Lee after handling it.

If the delay creates a meaningful conflict, unexpected cost, service scope change, safety issue, or Lee-specific preference question, pause at `confirm_unexpected_issue`.
...
Create a home-arrival reminder from the ETA.

Tell Lee Kylin is on the way home, include ETA, expected fee from the service result, and whether anything abnormal happened. Do not pay before home confirmation.
...
- Contact lookup.
- Reminder creation.
- Optional calendar changes unless Lee asks for them.
...
- If the selected grooming shop gives a different but compatible time, coordinate Driver and notify Lee.
- If the selected grooming shop changes service scope, ask Lee unless the user already authorized that scope.
- If Driver is unavailable, try to identify an alternative trusted transport contact or ask Lee.
...
- If an SMS reports an unexpectedly high fee, add-on, or price conflict, ask Lee before payment.
- If payment succeeds but accounting fails, report the accounting issue and keep the payment result visible.
- If Lee cancels, stop pending actions and summarize completed operational steps.
...
- Lee has received the final concise status update.
